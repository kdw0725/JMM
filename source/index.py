...
# DB서버와 연동
# -----------------------------
conn = pymysql.connect(host="jmm.cvxf3ahhscl8.us-east-2.rds.amazonaws.com", port=3306, user='JMM', passwd='1q2w3e4r',
    db='JMM', charset='utf8')
...
def choice_preference(region_kind):
    ...
    query = "예"
    total = cur.execute(
        "SELECT * FROM CHOICE WHERE CHO_TYPE='%s' and CHO_LOC='%s' and CHO_STORE='%s';" % (types, answer[0], store))
    choice_count = cur.execute(
        "SELECT * FROM CHOICE WHERE CHO_STORE='%s' and CHO_LOC='%s' and CHO_TYPE='%s' and CHO_QUERY='%s';" % (
        store, answer[0], types, query))
    print(choice_count)
    print(total)
    print(store)
    print(answer[0])
...
def storeInfo(region_kind):
    ...
    if (rescode == 200):
        ...
        if item:
            ...
            store_information += '🐥 자세히 보기 (🔍버튼을 눌러주세요): https://www.google.co.kr/maps/place/' + URLEncode(
                region) + '+' + URLEncode(
...
def get_menu_button(menu):
    ...
    menu_button = {'type': 'buttons', 'buttons': menu}
...
def get_answer(text, user_key):
    ...
    # Dialogflow에 요청
    # --------------------------------
    data_send = {'lang': 'ko', 'query': text, 'sessionId': user_key, 'timezone': 'Asia/Seoul'}

    data_header = {'Content-Type': 'application/json; charset=utf-8',
        'Authorization': 'Bearer eecec293709e4bf3b30396bf4a808876'# Dialogflow의 Client access token 입력
    }
...
def message():
    ...
    res = {'message': {'text': answer}}
    # ----------------------------메세지 설정 ------------------------------#

    if photo != '' and photo_width > 0 and photo_height > 0:
        res['message']['photo'] = {'url': photo, 'width': photo_width, 'height': photo_height}
    # ----------------------------사진 설정 --------------------------------#
